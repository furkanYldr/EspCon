...
bool ShapeOverlap_SAT_STATIC(polygon &r1, polygon &r2) {
  ...
  for (int shape = 0; shape < 2; shape++) {
    ...
    for (int a = 0; a < (int)poly1->p.size(); a++) {
      ...
      float currentOverlap = std::min(max_r1, max_r2) - std::max(min_r1, min_r2);
      if (currentOverlap < overlap) {
        overlap = currentOverlap;
        translationAxis = axisProj;
      }
    }
  }

  // İtme yönünün r1'i dışarı doğru atacak şekilde olmasını sağla
  vec2d d = { r1.pos.x - r2.pos.x, r1.pos.y - r2.pos.y };
  float dot = d.x * translationAxis.x + d.y * translationAxis.y;
  if (dot < 0) {
    translationAxis.x = -translationAxis.x;
    translationAxis.y = -translationAxis.y;
  }

  // Şekli çakışma ekseni doğrultusunda en kısa yoldan dışarı it
  r1.pos.x += overlap * translationAxis.x;
  r1.pos.y += overlap * translationAxis.y;
